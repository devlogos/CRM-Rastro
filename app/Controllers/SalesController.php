...
class SalesController
{
    ...
    public function setSendAudioFile($sendAudioFile)
    {
    ...
    public function setClientIsHolder($clientIsHolder)
    {
        $this->clientIsHolder = $clientIsHolder;
    }

    public function setStatusId($statudId)
    {
        $this->statusId = $statudId;
    }

    public function setSectorId($statudId)
    {
    ...
    {
        // crud task for insertion into the database
        return SalesModel::createImageFile(
            $this->saleId,
            $this->creationDate,
            $this->imageTimeStamp
        );
    ...
    public function read(
        ...
        if (count($sales) > 0) {
            foreach ($sales as $item) {
                ...
                $seller_url_image = $item['seller_url_image']
                    ? DOMAIN .
                        '/media/images/sellers/' .
                        $item['seller_url_image']
                    : ASSETS_PATH . '/img/seller-team.png';
                ...
                $its_scheduled = $item['its_scheduled'] ? true : false;
            ...
            if (count($paths) > 1) {
                ...
                foreach ($paths as $item) {
                    ...
                    if ($inc == 1) {
                        ...
                        $backHeight =
                            imagesy($refSizeImage) *
                            ceil(count($paths) / $docPerPage);
                ...
                imagecopymerge(
                    ...
                    0,
                    0,
                    imagesx($onlyOneImage),
                    imagesy($onlyOneImage),
                    100
                );
            }
