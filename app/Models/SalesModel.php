...
class SalesModel
{       
    public static function create($creation_date, $update_date, $code, $audio_time_stamp, $send_audio_file,$seller_id, $product_id, $district_id, $client_id, $note, $client_is_holder, $status_id, $reason_id, $shipping_id) {
        try {
            $sql = 'INSERT INTO sales (creation_date,update_date,code,audio_time_stamp,send_audio_file,seller_id,product_id,district_id,client_id,note,client_is_holder,sector_id,status_id,reason_id,shipping_id) VALUES (:creation_date,:update_date,:code,:audio_time_stamp,:send_audio_file,:seller_id,:product_id,:district_id,:client_id,:note,:client_is_holder,:sector_id,:status_id,:reason_id,:shipping_id);';
    ...
    public static function createImageFile($sale_id, $creation_date, $image_time_stamp) {
        try {
            $sql = 'INSERT INTO documents (sale_id, creation_date, image_time_stamp) VALUES (:sale_id, :creation_date, :image_time_stamp);';
    ...
    public static function read($companyId, $sellerId, $id = null, $dates, $status, $sectors, $limit) {
        $whereSeller = '';
        $whereSale = '';
        $whereAttr = '';
        ...
        if (!empty($dates)) {
            ...
            if (count($dates) > 1) {
                foreach ($dates as $date) {
                    if (!empty($date)) {
                        if ($inc == 0) {
                            $dates[$inc] = 'AND (date(A.creation_date) >= ' . "'{$date}'";
                        } else {
                            $dates[$inc] = ' AND date(A.creation_date) <= ' . "'{$date}'";
                        }
                        $inc++;
            ...
            $dates = sprintf("AND (date(A.creation_date) >= '%s' AND date(A.creation_date) <= '%s') ", $initialDate, $finalDate);
        }
        ...
        if (!empty($status)) {
            ...
            if (count($status) >= 1) {
                foreach ($status as $_status) {
                    if (!empty($_status)) {
                        if ($inc == 0) {
                            $status[$inc] = 'AND (A.status_id = ' . "{$_status}";
                        } else {
        ...
        if (!empty($sectors)) {
            if (count($sectors) >= 1) {
                foreach ($sectors as $sector) {
                    if (!empty($sector)) {
                        if ($inc == 0) {
                            $sectors[$inc] = 'AND (A.sector_id = ' . "{$sector}";
                        } else {
