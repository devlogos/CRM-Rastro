...
function readListProducts(categoryId, content) {
    if (parseInt(categoryId) === 0) {
        $(content).html('<div role="alert" class="alert alert-warning"><span>Selecione uma categoria!</span></div>');
    ...
    $.ajax({
        ...
        success: function (productsContent) {
            var option = '<select name="product_id" class="form-control select2" id="product_id">';
...
function readListCities(stateId, content, showDistrict, showmultiple, values) {
    var districtShow = typeof showDistrict === 'undefined' ? true : showDistrict;
    var multipleShow = typeof showmultiple === 'undefined' ? false : showmultiple;
    var valuesShow = typeof values === 'undefined' ? null : values;
    
    if (stateId.length === 0) {
        $(content).html('<div role="alert" class="alert alert-warning"><span>Selecione um estado!</span></div>');
    ...
    $.ajax({
        ...
        success: function (citiesContent) {
            if (citiesContent !== null) {
                var option = '<label for="city_id"><span class="mr-1 required">*</span>Cidade</label>';

                if (multipleShow) {
                    option += '<select name="city_id[]" class="form-control select2" id="city_id" multiple="multiple">';
                }
                else {
                    option += '<select name="city_id" class="form-control select2" id="city_id">';
                }
                ...
                $(document).ready(function () {
                    ...
                    if(valuesShow !== null){                        
                        $('#city_id').val(valuesShow).trigger('change');
                    }
                });
            }
        }
...
function readListDisctricts(cityId, content) {
    if (parseInt(cityId) === 0) {
        $(content).html('<div role="alert" class="alert alert-warning"><span>Selecione uma cidade!</span></div>');
    ...
    $.ajax({
        ...
        success: function (districtsContent) {
            if (districtsContent !== null) {
                var option = '<select name="district_id" class="form-control select2" id="district_id">';
                ...
                $(content).html('<div role="alert" class="alert alert-warning"><span>Não existem bairros para a cidade selecionada!</span></div>');
            }
        }
    });
}
