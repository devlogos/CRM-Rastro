            ...
            for (var item in result) {
                if (value === item) {
                    ...
                    if (value === "code") {
                        ...
                        if (result[item] === true) {
                            element.innerHTML =
                                '<span class="badge badge-success text-white">Titular</span>';
                        } else {
                            element.innerHTML = "";
                        }
                        ...
                        if (result[item] === "") {
                            element.innerHTML =
                                '<div role="alert" class="alert alert-warning"><span>Nenhuma observação realizada!</span></div>';
                        } else {
                            element.innerHTML = result[item];
                        }
                    } else {
                    ...
                    if (value === "map") {
                        ...
                        if (
                            ...
                            element.innerHTML =
                                '<div role="alert" class="alert alert-warning"><span><span><strong>Atenção! </strong>Geolocalização não identificada pelo dispositivo da venda.</span></div>';

                            return;
                        }

                        initMapDetailSale(
                            result["seller_url_image"],
                            result["latitude"],
                            result["longitude"],
                        ...
                        element.innerHTML =
                            '<div class="status-details-sale" style="background-color: ' +
                            result["status_color"] +
                            '"></div>';
                    } else if (value === "audio") {
                        ...
                        var initialDate = moment(
                            new Date(result["creation_date"])
                        );
                        var finalDate = moment(new Date(result["update_date"]));
                        var duration = moment.duration(
                            finalDate.diff(initialDate)
                        );

                        var minutes =
                            duration.get("minutes") <= 1
                                ? duration.get("minutes") === 0
                                    ? ""
                                    : "1 minuto e "
                                : duration.get("minutes") + " minutos e ";
                        var seconds =
                            duration.get("seconds") <= 1
                                ? duration.get("seconds") === 0
                                    ? ""
                                    : "1 segundo"
                                : duration.get("seconds") + " segundos";

                        var time = minutes + seconds;

                        element.innerHTML = time;
                    } else if (value === "documents") {
                        var images = jQuery.parseJSON(
                            JSON.stringify(result["image_time_stamp"])
                        );
                        ...
                        for (var index in images) {
                            ...
                            strImages =
                                strImages +
                                '<div class="col-sm-6 col-md-4 col-lg-3 col-doc col-doc-' +
                                result["id"] +
                                '" id="' +
                                colStartId +
                                '">' +
                                '<div class="custom-control custom-checkbox check_doc">' +
                                '<input type="checkbox" name="check_doc[]" data="' +
                                imageFilepath +
                                '" class="custom-control-input check-document" id="' +
                                docId +
                                '">' +
                        ...
                        if (inc === 0) {
                            element.innerHTML =
                                '<div role="alert" class="alert alert-warning"><span>Nenhum documento enviado! Verifique o motivo em caso de cancelamento.</span></div>';
                        } else {
                        ...
                        $(".convert-to-pdf").on("click", function(e) {
                            ...
                            if (paths !== "") {
                                ...
                                $("#result-convert").html(
                                    '<div role="alert" class="alert alert-warning"><span>Não existem documentos selecionados!</span></div>'
                                );
                            }
                        });
                        ...
                        element.innerHTML =
                            '<button class="btn btn-info finally-sale ml-1" style="background-color: ' +
                            background +
                            "; border-color: " +
                            background +
...
function initMapDetailSale(
    ...
    var icon = {
        path:
            "M236.925,0.124c-96.9,3.4-177.4,79-186.7,175.5c-1.9,19.3-0.8,38,2.6,55.9l0,0c0,0,0.3,2.1,1.3,6.1 c3,13.4,7.5,26.4,13.1,38.6c19.5,46.2,64.6,123.5,165.8,207.6c6.2,5.2,15.3,5.2,21.6,0c101.2-84,146.3-161.3,165.9-207.7 c5.7-12.2,10.1-25.1,13.1-38.6c0.9-3.9,1.3-6.1,1.3-6.1l0,0c2.3-12,3.5-24.3,3.5-36.9C438.425,84.724,347.525-3.776,236.925,0.124 z M243.825,291.324c-52.2,0-94.5-42.3-94.5-94.5s42.3-94.5,94.5-94.5s94.5,42.3,94.5,94.5S296.025,291.324,243.825,291.324z",
        fillColor: "#FF4D4D",
        fillOpacity: 1,
        scale: 0.098,
        strokeWeight: 0
