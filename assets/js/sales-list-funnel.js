...
$('.filter-sales').on('click', function () {
    ...
    statusId = (($('#status_id').val()).join(',')).replace('0,','');    
...
function readSalesList(dates) {
    ...
    $.ajax({
        type: 'GET',
        url: domain + "/sales/" + companyId + "/" + sellerId + "/0/" + initFinalDates + "/" + statusId + "/" + sectorsId + "/1,50",
        dataType: 'json',
        ...
        success: function (salesContent) {
            ...
            if (salesContent !== null) {
                    ...
                    'columnDefs': [
                        {'targets': 0,
                            'createdCell': function (td, cellData, rowData, row, col) {
                                var code = rowData.code === '' ? 'INDEFINIDO' : rowData.code;
                                $(td).prepend(code);
                            }
                        },
                        {'targets': 1,
                            ...
                            'createdCell': function (td, cellData, rowData, row, col) {
                                ...
                                var day = "<span class=\"badge badge-warning badge-date-sale mr-1\">" + ("00" + date.getDate()).slice(-2) + "</span>";
                                var month = "<span class=\"badge badge-secondary badge-date-sale mr-1 text-white\">" + ("00" + (date.getMonth() + 1)).slice(-2) + "</span>";
                                var year = "<span class=\"badge badge-dark badge-date-sale text-white\">" + date.getFullYear() + "</span>";

                                $(td).prepend(
                                        '<div class=\"no-wrap\">' + day + month + year + '<div>'
                                        );
                            }
                        },
                        {'targets': -1,
                            ...
                            'createdCell': function (td, cellData, rowData, row, col) {
                                $(td).prepend(
                                        "<button dataid=\"" + rowData.id + "\" class=\"btn btn-info select\" type=\"button\"><i class=\"icon-options\"></i></button>"
                                        );
                            }
                        }
                    ],
                ...
                status = status + '<div class=\"sectors-name d-none d-print-none d-sm-none d-md-none d-lg-block d-xl-block\">';
                status = status + '<span class=\"badge badge-info\">' + sectorsName + '</span>';
                status = status + '</div>';
                ...
                if (refreshSalesList) {
                    ...
                    $(tableSales).on('click', '.select', function () {
                        ...
                        if (!getKey('read_full_sale')) {
                            if (getKey('read_sample_sale')) {
                                ...
                                $('#details-sale').modal({backdrop: 'static', keyboard: false});
                            }
                        }
                        else {
                            clearInterval(intervalSales);
                    ...
                    $('.btn.btn-info.select').click(function () {
                        ...
                        $('select.owner_name').attr('style', 'visibility: hidden;');
                        $('i.status_edit').attr('style', 'visibility: inherit;');
                        $('select.status_name').attr('style', 'visibility: hidden;');
                    });
                }
...
function readSalesFunnel(dates) {
    ...
    $.ajax({
        type: 'GET',
        url: domain + "/sales/" + companyId + "/" + sellerId + "/0/" + initFinalDates + "/" + statusId + "/" + sectorsId + "/1,200",
        dataType: 'json',
        beforeSend: function (xhr) {
            $.toast({
                text: "Aguarde...",
        ...
        success: function (statusContent) {
            ...
            for (var key in statusContent) {
                var element = "#card-body-funnel-" + statusContent[key]['status_id'] + "";
                ...
                if (statusContent[key]['its_finished'] === true) {
                    finishedClass = "<i style=\"" + statusContent[key]['status_color'] + "\" class=\"material-icons\">check_box</i>";
                }

                // add too many cards to main card              
                strContent =
                        "<div class=\"card item-sale\" index=\"" + index + "\" id=\"card-item-funnel-" + statusContent[key]['id'] + "\" saleid=\"" + statusContent[key]['id'] + "\" draggable=\"true\" ondragstart=\"dragSale(event)\">" +
                            '<div class=\"card-body\">' +                            
                                '<span>' + finishedClass + '</span>' +                         
                ...
                $("#card-item-funnel-" + statusContent[key]['id']).on('click', function () {
                    var result = jQuery.parseJSON(localStorage.getItem('saleFunnel'));
                    var indexResult = $(this).attr('index');

                    localStorage.setItem("sale", JSON.stringify(result[parseInt(indexResult)]));
    ...
    function getWidthNav() {
        ...
        $('#funnel-content').attr('style', "width: " + widthFunnel + "px; padding-right: 5px !important;");
    }
}
        ...
        success: function (statusContent) {
            ...
            strContent =
                    '<div class="col-md-3 col-funnel">' +
                        "<div class=\"card card-over\" id=\"card-status-" + statusContent[1]['id'] + "\">" +
                            '<div class=\"card-header\">' +
                                "<div class=\"status-circle\" style=\"background-color: " + statusContent[1]['color'] + "\"></div>" +
                                '<h6 class=\"mb-0 upper\">' +
                                statusContent[1]['name'] +
                                '</h6>' +
                            '</div>' +
                            "<div id=\"card-body-funnel-" + statusContent[1]['id'] + "\" itsfinished=\"" + statusContent[1]['finished'] + "\" statusid=\"" + statusContent[1]['id'] + "\" ondrop=\"dropSale(event)\" ondrop=\"dropSale(event)\" ondragover=\"allowDropSale(event)\" class=\"card-body card-container\"></div>" +
                        '</div>' +
                    '</div>';
...
function allowDropSale(ev) {
    ...
    $(ev.target).closest('.card-over').attr('style','border: dashed 4px #7b7b7b !important;');
}
    ...
    if (cardClass === 'card-body card-container') {
        ...
        function update(ev,element) {
            $.ajax({
                ...
                data: {'statusId': statusId, 'saleId': saleId},
                dataType: 'html',
                beforeSend: function (xhr) {
                    xhr.setRequestHeader('token', token);
                    xhr.setRequestHeader('key', secretKey);
                ...
                success: function (content) {
                    if (parseInt(content) === 1) {
                        ...
                        if (itsFinishedCurr === true) {
                            $(element).contents().find('span').html('<i class=\"icon-check\"></i>');
                        }
        ...
        if (!getKey('read_full_sale')) {
            ...
            if (itsCancelled) {
                $.toast({
                    ...
                    text: [
                        'Esta venda encontra-se cancelada!',
                        'Deseja alterar seu status para <span style=\"text-transform: lowercase;\">' + statusName + '</span>?',                        
                        '<button id="yesStatusSale" style="cursor:pointer" class="btn btn-info next-sale mr-1 mt-1" type="button">SIM</button><button id="notStatusSale" style="cursor:pointer" class="btn btn-danger next-sale  mt-1" type="button">NÃO</button>'
                    ],
                    position: 'mid-center',
                    loader: true,
...
function updateSectorSale(sectorId, saleId) {
    $.ajax({
        ...
        data: {'sectorId': sectorId, 'saleId': saleId},
        dataType: 'html',
        beforeSend: function (xhr) {
            xhr.setRequestHeader('token', token);
            xhr.setRequestHeader('key', secretKey);
...
function nextSale() {
    ...
    if (rowSaleIndex < countSales-1) {
        ...
        var btnDetailsNext = "button[dataid='" + getSale(rowSaleIndex, 'id') + "']";
        var btnDetailsPrevious = "button[dataid='" + getSale(rowSaleIndex-1, 'id') + "']";
        ...
        if (rowSaleIndex >= 1) {
            ...
            var windowHeight = $(window).height().toFixed(0);
            var offSet = $(btnDetailsNext).offset().top;

            if (offSet > windowHeight) {
                var $target = $('html,body');
                $target.animate({scrollTop: offSet - 10}, 1000);
            }
        }
    }    
}
        ...
        if (getKey('read_sample_sale')) {
            $('#sales > tbody  > tr > td > button.select').each(function () {                
                if ($(this).attr('dataid') !== getSale(0,'id')) {
                    $(this).closest('td').html("<button dataid=\"" + $(this).attr('dataid') + "\" class=\"btn btn-info select-over disabled\" type=\"button\"><i class=\"icon-lock\"></i></button>");                    
                }
            });
        }
        else{
            $('#sales > tbody  > tr > td > button.select').each(function () {
                $(this).closest('td').attr('style','text-align: center;');
                $(this).closest('td').html('<i class=\"icon-lock no-access\"></i>');
            });
        }
    }
}
...
function valNewSale() {
    // validate fields
    var creationDate = $('#creation_date').val() === '' ? null : $('#creation_date').val();
    var code = $('#code').val() === '' ? null : $('#code').val();
    var productId = typeof $('#product_id').val() === 'undefined' ? null : $('#product_id').val();
    var districtId = typeof $('#district_id').val() === 'undefined' ? null : $('#district_id').val();
    var clientId = typeof $('#client_id').val() === 'undefined' || $('#client_id').val().length === 0 || $('#client_id').val()[0] === '0' ? null : $('#client_id').val();
