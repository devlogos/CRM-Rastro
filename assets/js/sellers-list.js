        ...
        success: function (sellersContent) {
            if (sellersContent !== null) {
                    ...
                    'columnDefs': [
                        {'targets': 0,
                            ...
                            'createdCell': function (td, cellData, rowData, row, col) {
                                var strCheck = "<div class=\"custom-control custom-control-seller custom-checkbox\">" +
                                        "<input type=\"checkbox\" index=" + incIndex + " name=\"check[]\" id=\"row-" + rowData.id + "\" class=\"custom-control-input seller-chk\">" +
                                        "<label class=\"custom-control-label\" for=\"row-" + rowData.id + "\">&nbsp;</label>" +
                                        "</div>";
                        ...
                        {'targets': 2,
                            ...
                            'width': '60px',
                            'createdCell': function (td, cellData, rowData, row, col) {
                                var imageProfile = domain + '/media/images/sellers/' + rowData.url_image;

                                if (rowData.url_image === null) {
                                    imageProfile = domain + '/assets/img/' + 'seller-team.png';
                                }

                                $(td).prepend(
                                        "<img src=\"" + imageProfile + "\" class=\"rounded-circle img-fluid image-profile\" />"
                                        );
                            }
                        },
                        {'targets': 3,
                            ...
                            'createdCell': function (td, cellData, rowData, row, col) {
                                ...
                                if (salesAmout > 0) {
                                    $(td).prepend(
                                            "<span class=\"badge badge-dark badge-date-seller\">" + salesAmout + "</span>"
                                            );
                                }
                                else {
                                    $(td).prepend(
                        ...
                        {'targets': 4,
                            ...
                            'createdCell': function (td, cellData, rowData, row, col) {
                                ...
                                if (totalSalesMade > 0) {
                                    $(td).prepend(
                                            "<span class=\"badge badge-success badge-date-seller\">" + totalSalesMade + "</span>"
                                            );
                                }
                                else {
                                    $(td).prepend(
                ...
                if (refreshSellersList) {
                    ...
                    var buttons = '<div class="sellers-buttons-action">' +
                    ...
                    $('.notification').on('click', function () {
                        ...
                        $('.seller-chk').each(function () {
                            if ($(this).prop('checked')) {
                                ...
                                var firebaseToken = getSeller(index, 'firebase_token');

                                var input = "<input type=\"checkbox\" checked name=\"firebase_token[]\" value=\"" + firebaseToken + "\">";

                                if (firebaseToken !== null) {
                                    $('.token-group').append(input);

                                    badges = badges + "<span class=\"badge badge-info mr-2\">" + seller + "</span>";
                                }
                                else {
                                    badges = badges + "<span class=\"badge badge-secondary mr-2\">" + seller + "</span>";
                                }
                            }
                        });
                    ...
                    $(tableSellers).on('click', '.select', function () {
                        ...
                        localStorage.setItem('seller', getSeller(rowSellerIndex));
...
function clearFields() {
    ...
    $('#send_after_sale').prop('checked', false);
}
...
function valNewSeller() {
    ...
    var cityId = typeof $('#city_id').val() === 'undefined' || $('#city_id').val().length === 0 || $('#city_id').val()[0] === '0' ? null : $('#city_id').val();
    var recordingTime = $('#recording_time').val() === '' ? null : $('#recording_time').val();
    var sampleRate = $('#sample_rate').val() === '' ? null : $('#sample_rate').val();
    var bitsPerSample = $('#bits_per_sample').val() === '' ? null : $('#bits_per_sample').val();
